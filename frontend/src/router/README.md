...
This directory contains the Vue Router configuration for the DreamStudio frontend application. The router implements a comprehensive navigation system with authentication guards, role-based access control, and lazy loading.
...
| `/dashboard` | DashboardView | User dashboard               |
...
| `/admin/ops`        | OpsDashboardView   | Ops monitoring                  |
...
<template>
  ...
  <router-link :to="{ path: '/profile', query: { tab: 'security' } }"> Profile </router-link>
</template>
```
