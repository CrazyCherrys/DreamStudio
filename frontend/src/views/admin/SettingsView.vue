    ...
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6">
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.generationTimeout.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.generationTimeout.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <!-- Loading State -->
            <div v-if="generationTimeoutLoading" class="flex items-center gap-2 text-gray-500">
              <div class="h-4 w-4 animate-spin rounded-full border-b-2 border-primary-600"></div>
              {{ t('common.loading') }}
            </div>
            ...
            <template v-else>
              ...
              <div>
                <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.generationTimeout.imageTimeout') }}
                </label>
                <div class="flex items-center gap-2">
                  <input
                    v-model.number="generationTimeoutForm.image_timeout_seconds"
                    type="number"
                    min="10"
                    max="600"
                    class="input w-32"
                  ...
                  <span class="text-sm text-gray-500 dark:text-gray-400">
                    {{ t('admin.settings.generationTimeout.seconds') }}
                  </span>
                </div>
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.generationTimeout.imageTimeoutHint') }}
                </p>
              </div>
              ...
              <div class="border-t border-gray-100 pt-4 dark:border-dark-700">
                <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.generationTimeout.videoTimeout') }}
                </label>
                <div class="flex items-center gap-2">
                  <input
                    v-model.number="generationTimeoutForm.video_timeout_seconds"
                    type="number"
                    min="10"
                    max="600"
                    class="input w-32"
                ...
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.generationTimeout.videoTimeoutHint') }}
                </p>
              </div>
              ...
              <div class="flex justify-end border-t border-gray-100 pt-4 dark:border-dark-700">
                <button
                  type="button"
                  @click="saveGenerationTimeoutSettings"
                  :disabled="generationTimeoutSaving"
                  class="btn btn-primary btn-sm"
                >
                  <svg
                    v-if="generationTimeoutSaving"
                    class="mr-1 h-4 w-4 animate-spin"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                  ...
                  {{ generationTimeoutSaving ? t('common.saving') : t('common.save') }}
                </button>
              </div>
            </template>
          </div>
...
// Generation Timeout 状态
const generationTimeoutLoading = ref(true)
const generationTimeoutSaving = ref(false)
const generationTimeoutForm = reactive({
...
// Generation Timeout 方法
async function loadGenerationTimeoutSettings() {
  generationTimeoutLoading.value = true
  try {
    const settings = await adminAPI.settings.getGenerationTimeoutSettings()
    Object.assign(generationTimeoutForm, settings)
  } catch (error: any) {
    console.error('Failed to load generation timeout settings:', error)
  } finally {
    generationTimeoutLoading.value = false
  }
}

async function saveGenerationTimeoutSettings() {
  generationTimeoutSaving.value = true
  try {
    const updated = await adminAPI.settings.updateGenerationTimeoutSettings({
      image_timeout_seconds: generationTimeoutForm.image_timeout_seconds,
      video_timeout_seconds: generationTimeoutForm.video_timeout_seconds
    })
    Object.assign(generationTimeoutForm, updated)
    appStore.showSuccess(t('admin.settings.generationTimeout.saved'))
  } catch (error: any) {
    appStore.showError(
      t('admin.settings.generationTimeout.saveFailed') + ': ' + (error.message || t('common.unknownError'))
    )
  } finally {
    generationTimeoutSaving.value = false
  }
}
