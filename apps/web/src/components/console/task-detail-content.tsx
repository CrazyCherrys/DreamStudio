...
export function ConsoleTaskDetailContent({ taskId }: { taskId: string }) {
  ...
  useEffect(() => {
    async function loadTask() {
      ...
      setError(null);
      try {
        const payload = await fetchImageTask(taskId);
      ...
      <section className="ds-card admin-panel p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-4 text-2xl font-black">任务详情</h2>
          </div>
          <Link className="ds-button ds-button-secondary" href="/console/tasks">
            返回任务列表
          </Link>
        </div>
      </section>
  ...
  return (
    <div className="grid gap-5">
      <section className="ds-card admin-panel p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="break-words text-2xl font-black">{task.prompt_summary}</h3>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="ds-card admin-panel p-5">
