#!/bin/bash
#
# DreamStudio Installation Script
# DreamStudio 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/CrazyCherrys/DreamStudio/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="CrazyCherrys/DreamStudio"
INSTALL_DIR="/opt/dreamstudio"
SERVICE_NAME="dreamstudio"
SERVICE_USER="dreamstudio"
CONFIG_DIR="/etc/dreamstudio"
...
declare -A MSG_ZH=(
    ...
    # Installation
    ["install_title"]="DreamStudio 安装脚本"
    ...
    # Completion
    ["install_complete"]="DreamStudio 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 DreamStudio 服务："
    ...
    ["not_installed"]="DreamStudio 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 DreamStudio。"
    ...
    ["uninstall_complete"]="DreamStudio 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 DreamStudio"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 DreamStudio"
    ...
    ["server_config_desc"]="配置 DreamStudio 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="DreamStudio Installation Script"
    ...
    # Completion
    ["install_complete"]="DreamStudio installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start DreamStudio service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading DreamStudio..."
    ...
    ["not_installed"]="DreamStudio is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove DreamStudio from your system."
    ...
    ["uninstall_complete"]="DreamStudio has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install DreamStudio"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove DreamStudio"
    ...
    ["server_config_desc"]="Configure DreamStudio server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/dreamstudio" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/dreamstudio" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    local archive_name="dreamstudio_${LATEST_VERSION}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/dreamstudio" "$INSTALL_DIR/dreamstudio"
    chmod +x "$INSTALL_DIR/dreamstudio"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/dreamstudio"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/dreamstudio.service << EOF
[Unit]
Description=DreamStudio - AI API Gateway Platform
Documentation=https://github.com/CrazyCherrys/DreamStudio
...
User=dreamstudio
Group=dreamstudio
WorkingDirectory=/opt/dreamstudio
ExecStart=/opt/dreamstudio/dreamstudio
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=dreamstudio
...
ProtectHome=true
PrivateTmp=true
ReadWritePaths=/opt/dreamstudio
...
start_service() {
    ...
    if systemctl start dreamstudio; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u dreamstudio -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable dreamstudio 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status dreamstudio"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u dreamstudio -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart dreamstudio"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop dreamstudio"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if DreamStudio is installed
    if [ ! -f "$INSTALL_DIR/dreamstudio" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/dreamstudio" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet dreamstudio; then
        print_info "$(msg 'stopping_service')"
        systemctl stop dreamstudio
    fi

    # Backup current binary
    cp "$INSTALL_DIR/dreamstudio" "$INSTALL_DIR/dreamstudio.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/dreamstudio.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/dreamstudio"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start dreamstudio

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: DreamStudio must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/dreamstudio" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="dreamstudio.backup.${current_version}"
        else
            backup_name="dreamstudio.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/dreamstudio" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop dreamstudio 2>/dev/null || true
    systemctl disable dreamstudio 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/dreamstudio.service
    systemctl daemon-reload
