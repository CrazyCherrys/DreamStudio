...
- 最新 batch 顶部倾斜小封面优先使用任务公开返回的 `reference_assets`；只要任务带参考图，就显示前 2-3 张参考图叠卡，并在 hover/focus 时提示“点击可复用照片”；点击后只把这组参考图整组替换到当前输入区上传照片区域，不回填 Prompt；没有参考图时才回退到首张结果图。
- 最新 batch 顶部的结果摘要只表达结果图数量或返回进度，不再和快捷参数里的 `count` 重复显示“张数: n”。
