...
- Prompt 子容器为无边框沉浸式整块容器，参考图入口与 Prompt 输入区合并在同一块面内；Prompt 输入框本身无描边，聚焦时只显示浅底高亮。
- 生成按钮以纸飞机发送图标呈现；其左侧为回车模式切换按钮，默认浅色表示回车直接发送，点击后蓝色表示回车换行，并使用 `100ms` 颜色过渡。该偏好保存在浏览器本地。
- 快捷参数从当前模型 `parameter_schema` 生成；配置了张数和尺寸但未配置分辨率时，只显示张数和尺寸。
- `尺寸` 表示画幅比例，如 `1:1`、`16:9`、`9:16`；`分辨率` 表示像素规格，如 `1024x1024`。
...
- `/studio` 发送入口为纸飞机图标，回车模式切换按钮可在回车发送和回车换行之间切换并保存在浏览器本地。
