...
- 参考图上传区未上传时只显示加号上传图标，不显示“参考图”文字标签；上传后在更大的左侧预览区内默认层叠展示缩略图，末尾常驻弱化的加号添加卡片，保留轻量 `n/8` 状态提示和右下角极小加号弱提示，且不遮挡输入区。
- 底部操作 rail 第一项额外提供紧凑参考图按钮，和左侧预览共用同一个上传入口；空状态显示“添加”，有图时显示 `n/max`，上传中显示“上传中”，不支持参考图时禁用。
