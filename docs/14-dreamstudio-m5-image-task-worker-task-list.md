...
- Prompt 子容器为无边框沉浸式整块容器，参考图入口与 Prompt 输入区合并在同一块面内；快捷参数 chips 内嵌在输入区底部，不再独立悬浮在输入框外；Prompt 输入框本身无描边，聚焦时保持近乎静默的无边框效果，不出现描边、ring 或明显高亮底色。
...
- 参考图上传区未上传时只显示加号上传图标，不显示“参考图”文字标签；上传后在更大的左侧预览区内默认层叠展示缩略图，末尾常驻弱化的加号添加卡片，保留 `n/8` 数量胶囊和右下角极小加号弱提示，且不遮挡输入区。
