...
- 模型未配置的快捷参数不显示、不占位；例如只配置张数和尺寸时，不显示分辨率。
- Studio 中 `尺寸` 表示画幅比例，例如 `1:1`、`16:9`、`9:16`；`分辨率` 表示像素规格，例如 `1024x1024`。
...
- 配置一个包含 `prompt_style`、`size`、`n` 的参数 Schema，其中 `size` 用于画幅比例；需要像素规格时另配分辨率字段。
- 禁用模型后，普通用户列表不再展示。
