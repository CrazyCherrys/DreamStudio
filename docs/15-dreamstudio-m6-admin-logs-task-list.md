...
- 用户列表支持关键字、状态筛选和分页。
- `/admin/users` 行内“详情/管理”打开当前页用户详情弹窗，不从列表跳转到新页面。
- 用户详情弹窗展示基础信息、状态、new-api 配置状态、最近登录时间和会话摘要。
- 用户详情弹窗支持通过二级弹窗确认启用、禁用、软删除、重置密码、代用户配置 new-api 和清空用户 new-api 密钥。
- `/admin/users/[user_id]` 保留为兼容直达页，复用同一套用户详情面板。
- 请求日志列表支持状态、模型、用户、时间筛选。
- 请求日志详情展示任务、模型、状态、HTTP 状态、耗时、错误摘要、Prompt 摘要、脱敏参数。
- 请求日志详情展示 adapter/profile、脱敏最终请求、上游响应摘要和 profile 排障提示。
- 请求日志详情提供 reveal Prompt 和 reveal 参数按钮，先通过弹窗确认审计提示，再在弹窗内临时展示完整敏感内容。
...
- 管理员可以在 `/admin/users` 当前页弹窗中查看用户详情。
- 管理员可以禁用用户并使会话失效。
- 管理员可以启用用户。
- 管理员可以软删除用户。
- 管理员可以重置用户密码并使旧会话失效。
