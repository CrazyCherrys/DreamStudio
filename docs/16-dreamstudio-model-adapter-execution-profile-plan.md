...
兼容期可以保留旧字段，但新增逻辑优先读取默认 `ExecutionProfile`。

### 6.2 ExecutionProfile
...
## 7. Parameter Schema v2

扩展现有 `parameter_schema`，保持旧字段兼容。
...
- 没有 profile snapshot 的老任务继续走 legacy OpenAI Image client 路径。
...
- 为现有 `AiModel` 自动生成默认 active profile。
- `openai_image_generations` 迁移为 `openai_images_generation` profile。
- `openai_image_edits` 迁移为 `openai_images_edit` profile。
- 创建任务时快照 profile revision。
- Worker 优先读取 profile snapshot，保留 legacy fallback。
- 普通模型接口兼容旧字段，同时增加 `default_execution_profile`。
...
- 旧 schema 自动兼容为 `advanced` 或按迁移规则补充 `ui.slot`。
...
- 更新图片任务 Worker 文档。
- 新增模型接入指南。
- 支持 profile/revision 导入和导出 JSON。
...
### 16.2 新增第三方 OpenAI-compatible 模型
...
- 旧 OpenAI Image 生成和编辑任务继续可用。
- 旧模型自动拥有默认 execution profile 和 active revision。
- 未声明参数不会发送到上游。
