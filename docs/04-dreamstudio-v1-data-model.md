...
保存管理员维护的模型目录。当前开发阶段仍保留部分旧执行字段，后续新任务会逐步改为读取默认 execution profile。
...
- 只有启用模型展示给普通用户。
- 旧链路的模型参数暂时以 `parameter_schema` 为准；新链路会以默认 active profile revision 的 `parameter_schema` 为准。
...
### 5.5 ai_model_execution_profiles
...
- 新任务最终必须从模型默认启用 profile 中选择 active revision。
- 初始化脚本已创建开发默认 `gpt-image-2` 图片模型，并给启用的 OpenAI generation 图片模型补齐默认 profile。
- 任务链路切换到 active profile revision 和 profile snapshot 仍在后续阶段实现。

### 5.6 ai_model_execution_profile_revisions
...
- 同一个 profile 同一时间只能发布一个 active revision；当前阶段暂未添加数据库级 partial unique 约束，发布逻辑在后续服务层实现。
- 任务创建后保存 revision 快照，不应被后续 active revision 变更影响。
...
- 当前旧链路使用模型、接口、服务地址和参数快照执行；profile 快照字段已预留，后续阶段会让新任务使用完整 profile/revision 快照执行。
