...
const (
	...
	githubRepo     = "CrazyCherrys/DreamStudio"
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".dreamstudio-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "dreamstudio")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "dreamstudio" || baseName == "dreamstudio.exe" {
